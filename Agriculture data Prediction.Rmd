...
# Read the dataset
data <- read.csv("/Users/xitijmoradiya/Desktop/India Agriculture Crop Production.csv")
#View(data)
# Remove rows with missing data
na.omit(data)

# Remove entries with season "Whole year"
data <- data %>% filter(Season != "Whole year")
data <- data %>% filter(Production.Units != "Nuts")

# Define conversion factors
conversion_factor <- c("Bales" = 5)

#names(data)

data$Production_tonnes <- ifelse(is.na(data$Production.Units), NA, 
                                 ifelse(data$Production.Units %in% names(conversion_factor), 
                                        as.numeric(data$Production) * conversion_factor[data$Production.Units], 
                                        as.numeric(data$Production)))
#View(data)

# Remove original Production column
data$Production <- NULL
data$Production.Units <- NULL
...
data <- data %>% na.omit()
# Group the less common categories into an "Other" category
top_crops <- data %>%
  group_by(Crop) %>%
  summarize(count = n()) %>%
  arrange(desc(count)) %>%
...
data$Crop <- ifelse(data$Crop %in% top_crops, as.character(data$Crop), "Other")

# Convert the Crop column to a factor variable
...
rf_model <- randomForest(Crop ~ ., data = train_data)
...
top_10_seasonal_crops <- data %>%
  group_by(Season) %>%
  slice_max(order_by = Production_tonnes, n = 10)

# Print the result
print(top_10_seasonal_crops)
View(top_10_seasonal_crops)
...
top_10_seasonal_crops <- data %>%
  ...
  slice_head(n = 10) %>%
  select(Season, Crop, Production_tonnes, State)
...
summer_data <- data %>% filter(Season == "Winter")

# Get top 10 crops for summer season
top_crops_summer <- summer_data %>%
  group_by(Crop) %>%
  summarize(Production_tonnes = sum(Production_tonnes, na.rm = TRUE)) %>%
  arrange(desc(Production_tonnes)) %>%
  head(10)

# Create bar plot
ggplot(top_crops_summer, aes(x = Crop, y = Production_tonnes)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Top 10 Crops by Production in Tonnes (Winter Season)",
       x = "Crop", y = "Production (Tonnes)") +
